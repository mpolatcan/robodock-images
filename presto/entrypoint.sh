...
# -------------------------- Load Cassandra Connector Configs ---------------------------
...
if [ "${PRESTO_REDIS_NODES}" != "NULL" ]
    then
        echo "connector.name=redis-cluster" >> ${PRESTO_CONNECTORS}/redis.properties
        loadConfig "redis-cluster.nodes" ${PRESTO_REDIS_NODES} "${PRESTO_CONNECTORS}/redis-cluster.properties"
        loadConfig "redis-cluster.table-names" ${PRESTO_REDIS_TABLE_NAMES} "${PRESTO_CONNECTORS}/redis-cluster.properties"
        loadConfig "redis-cluster.scan-count" ${PRESTO_REDIS_SCAN_COUNT} "${PRESTO_CONNECTORS}/redis-cluster.properties"
        loadConfig "redis-cluster.key-prefix-schema-table" ${PRESTO_REDIS_KEY_PREFIX_SCHEMA_TABLE} "${PRESTO_CONNECTORS}/redis-cluster.properties"
        loadConfig "redis-cluster.key-delimiter" ${PRESTO_REDIS_KEY_DELIMITER} "${PRESTO_CONNECTORS}/redis-cluster.properties"
        loadConfig "redis-cluster.hide-internal-columns" ${PRESTO_REDIS_HIDE_INTERNAL_COLUMNS} "${PRESTO_CONNECTORS}/redis-cluster.properties"
        loadConfig "redis-cluster.database-index" ${PRESTO_REDIS_DATABASE_INDEX} "${PRESTO_CONNECTORS}/redis-cluster.properties"
        loadConfig "redis-cluster.password" ${PRESTO_REDIS_PASSWORD} "${PRESTO_CONNECTORS}/redis-cluster.properties"
fi
# ----------------------------------------------------------------------------
